 ...
 */
export function normalizeTranscript(raw: string): { transcript: string; turns: TranscriptTurn[] } {
  let text = raw
    .replace(/\r\n|\r/g, "\n")
    .replace(/[ \t]+/g, " ")
    .trim();
 ...
 */
export function fixSpeakerAttribution(
...
export const MEETING_SYSTEM_PROMPT = `You are a meeting analysis engine for Indonesian business conversations.
...
export function parseMeetingJsonResponse(raw: string): unknown {
...
export function mergeParticipantsBySpeaker(participants: ParticipantShape[]): ParticipantShape[] {
  const bySpeaker = new Map<string, ParticipantShape>();
  for (const p of participants) {
